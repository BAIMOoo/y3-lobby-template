...
2. 按函数参数提示传入玩法 ID、人数、目标关卡或参数表等数据。
3. 将函数返回值接收为 ECA `TABLE` 变量。
4. 读取返回表里的 `accepted`、`reason`、`code`、`request_id`。

例如，调用“大厅服务 - 建立连接”时填写正整数玩法固定 ID；当前示例项目使用 `190356`，迁移到自己的项目时填写目标项目上传平台后供 BOB 服务识别的玩法固定 ID，不要用运行时 `game_play_id` 代替。如果当前关卡是目标玩法关卡，再将“是否在游戏关卡”设为 `true`。
