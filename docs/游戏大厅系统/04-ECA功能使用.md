...
2. 在主关卡的 UI 编辑器导入只供 ECA 作者使用的 `大厅服务ECA示例UI.upui`。
3. 在主关卡导入迁移包中的 `大厅服务ECA触发包.zip`。
4. 确认导入后存在两张 ECA 示例画板、自定义事件 `大厅服务请求完成`（字典参数 `回调数据`）、`大厅服务状态变化`（字典参数 `事件数据`）、26 个“大厅服务 - ”函数和三个分类触发器目录。
...
- 16 个大厅 UI 触发器和 8 个副本 UI 触发器；
- 完整的 26 个“大厅服务 - ...”函数，以及函数测试和状态事件示例。

编辑器触发器树使用“大厅服务/大厅UI”“大厅服务/副本UI”“大厅服务/接口测试”三组目录；生成 DSL 的 `folder` 字段是目录结构的来源，重新生成时不会把触发器写回根目录。
...
大厅成员列表的四个“转移”按钮统一注册 `eca_lobby_transfer` 事件，四个“移出”按钮统一注册 `eca_lobby_kick` 事件。每类操作各由一个顶层触发器读取事件源控件名并解析成员序号；请求返回非空 `request_id` 时，才动态注册一次性完成回调，收到匹配的完成事件后立即注销。修改成员管理逻辑时只维护对应的共享触发器，不要再按成员槽位复制顶层触发器。
