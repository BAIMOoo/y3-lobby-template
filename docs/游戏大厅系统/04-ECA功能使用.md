...
退出游戏和普通异步请求的取消结果通过已有的 `大厅服务请求完成` 事件接收，不新增 ECA 对外调用接口。同房分流、加入口令和返回大厅是请求提交型跨图接口，不产生完成事件。
...
上例中，`args[1]` 是正整数玩法 ID。当前示例项目使用 `190356`；迁移到自己的项目时，填写目标项目对应的玩法 ID。这个值不是调试参数，也不是 UUID 格式的 `map_id`。
...
同房分流、加入口令和返回大厅不依赖 `大厅服务请求完成` 是否存在。三项接口立即返回的 `request_id` 为空，`result_data.cross_map_tracking = 'degraded'`，不会进入完成事件等待；必须在新地图加载后重新查询状态。
...
同房分流、加入口令和返回大厅的立即结果会包含 `cross_map_tracking = 'degraded'` 和 `entered_target = 'unknown'`。`platform_requested = true` 只表示引擎方法已调用，不表示平台最终接受。返回大厅不执行退出清理，也不返回 `cleanup_pending`。跨房合流仍沿用原完成事件契约。
...
3. 对同房分流、加入口令和返回大厅，`accepted = true` 后显示“请求已发送/等待切图”，不要保存或等待 `request_id`。
4. 对其他异步接口，保存 `request_id`，并在 `大厅服务请求完成` 的 `回调数据` 中匹配。
5. 跨图后在新地图重新查询状态；普通异步接口根据 `success`、`reason`、`result_data` 更新界面。
...
退出清理正在处理时，新的“大厅服务 - 建立连接”会返回 `connection_closing`，队伍、匹配、聊天、同房分流、跨房合流、口令等操作会返回 `terminal_in_progress`，第二个终态请求会返回 `terminal_locked`。
