...
`eca-example-map` 不是把每张关卡各放一份 UI。当前实现由 `EntryMap` 统一持有 `EcaLobbyExample` 和 `EcaDungeonExample` 两套 UI，以及 22 个“ECA大厅UI - ...”触发器和 8 个“ECA副本UI - ...”触发器。运行模式为 `0` 或 `1001` 时显示大厅 UI；其他模式显示副本 UI。
...
仅在 `eca-example-map` 中还可参考 `maps/EntryMap/ui/EcaLobbyExample.json`、`maps/EntryMap/ui/EcaDungeonExample.json` 和 `maps/EntryMap/global_trigger/trigger/ECA*UI - *.json`。这些是当前模板项目的示例调用层，不属于通用迁移文件。
