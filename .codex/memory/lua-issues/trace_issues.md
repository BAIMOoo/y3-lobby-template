...
## 11. 本地调试默认环境不能覆盖显式服务端点

- 时间：2026-08-12
- 场景：地图入口已调用 `y3.lobby.connect(190356, false, 'pre')`，但本地多开日志仍显示连接 QA 地址 `42.186.215.253:8092`，多人私人副本请求返回 `dungeonLevelConf is nil`。
- 根因：客户端端点解析先判断 `y3.game.is_debug_mode()`，本地调试时直接选择 QA，导致调用方显式传入的 `endpoint_env = 'pre'` 永远没有机会生效。
- 解决方案：端点选择顺序改为“显式 `endpoint_env` > 本地调试或平台 QA > 平台 pre > prod”。显式传入 `pre` 时连接 `42.186.213.132:8092`；未传环境时，本地调试仍默认连接 QA。
- 预防建议：端点合同测试必须同时覆盖“本地调试 + 显式 pre”和“本地调试 + 未显式环境”，并直接断言目标 IP，不能只断言环境名。
