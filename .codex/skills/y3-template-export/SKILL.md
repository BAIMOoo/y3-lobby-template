...
description: |
  Y3 功能模板导出器。把项目中现有模块自动化打包为 `.codex/templates/<id>/` 三件套
  （`logic.lua` + `<id>.upui` + `ReadMe.md`），并追加 `draft` 条目到 `.codex/templates/ReadMe.md`。
...
把"现有模块"按 `.codex/templates/ReadMe.md §6` 自动化打包为 `draft` 模板候选。
...
| 在 `.codex/templates/ReadMe.md` 追加条目 | lua 风格审查 → `y3-lua-review` |
...
| 类型 | 路径 | 方式 |
|---|---|---|
| 白名单 | `.codex/templates/<id>/` | 新建目录 |
| 白名单 | `.codex/templates/ReadMe.md` | **仅追加** |
...
激活后**第一句话必须是**：
> "好，我将把 `<module>` 打包为模板。按 `.codex/templates/ReadMe.md §6` 的 9 步流程执行，其中 4 个人工卡点需要你确认。请先告诉我：**模板等级**（A/B/C/D，见 §3）？**模板 ID 是什么**（kebab-case，带等级前缀如 `c-pick-one-of-many`）？**模块入口文件在哪里**？**模板的唯一主功能是什么**（一句话）？"
...
- [ ] `.codex/templates/<id>/` **不存在**（避免覆盖既有模板）
...
```
output_path          = <ABS .codex/templates/<id>/>   必填，绝对路径
...
### 14 字段（严格对齐 `.codex/templates/ReadMe.md §1`）
...
| 名称 | <中文名，例：HUD 状态栏模板> |
| 路径 | `.codex/templates/<template-id>/` |
| 状态 | `draft` |
| 版本 | `v0.1.0` |
| 能力标签 | `<tag1>`, `<tag2>`, `<tag3>` |
| 适用场景 | <步骤 1 范围描述正式版> |
...
**定位策略**：在 `.codex/templates/ReadMe.md` 中找 `## 7. 当前可用模板`
...
✅ 模板 `<template-id>` 已打包为 draft：
   - 目录：.codex/templates/<template-id>/
   - 登记：.codex/templates/ReadMe.md
...
- [ ] `.codex/templates/<id>/ReadMe.md` 存在且 14 字段齐全（含「等级」「入口签名」字段）
- [ ] `.codex/templates/<id>/logic.lua` 存在，是唯一 `.lua` 文件
- [ ] `logic.lua` 头部含 `@template-id` / `@grade` / `@version` / `@entry`
- [ ] **等级前缀一致**：B→`b-*` / C→`c-*` / D→`d-*` / A→可省略
- [ ] `.codex/templates/<id>/<id>.upui` 存在 + 体积合理（纯 Lua 模板可豁免）
...
- [ ] `.codex/templates/ReadMe.md` 比流程前多一个条目，其它内容字节级不变
...
`.codex/templates/ReadMe.md` —— 本 skill 所有规则均派生自此文件 §0-§6（§0 等级机制 / §1-§6 模板规范）。
冲突时以 `.codex/templates/ReadMe.md` 为准。
