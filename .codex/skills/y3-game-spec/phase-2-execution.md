...
python3 .codex/skills/y3-obj-edit/scripts/y3_obj_edit.py \
...
提取需求特征:
  ...
  - 逻辑对象（路径航点数/区域数量/特殊对象）
    │
    ▼
扫描模板库 .codex/skills/y3-terrain-template/library/
...
> 存放位置：`.codex/skills/y3-terrain-template/library/`
...
| 约束项 | 强制要求 | 禁止行为 |
|-------|---------|---------|
| **模板来源** | 必须使用 `.codex/skills/y3-terrain-template/library/<template-name>/` 原始模板 | ❌ 禁用其他模板，禁用 `y3-gen-terrain-from-image` 重生成 |
...
```
1. python .codex/skills/y3-terrain-template/scripts/import_terrain_template.py <template-name> EntryMap
