...
log.info('[MapName001][BobTestUI] test_ui.lua module loaded, mode=' .. tostring(current_mode()))
...
local function schedule_build(player)
    log.info('[MapName001][BobTestUI] schedule_build called, player=' .. tostring(player))
    y3.ltimer.wait_frame(5, function()
        log.info('[MapName001][BobTestUI] delayed build callback fired')
...
y3.game:event('玩家-加入游戏', function(_, data)
    y3.player.with_local(function(local_player)
        log.info(string.format(
            '[MapName001][BobTestUI] player-join event: event_player=%s local_player=%s is_local=%s',
            tostring(data.player),
            tostring(local_player),
            tostring(data.player == local_player)))
